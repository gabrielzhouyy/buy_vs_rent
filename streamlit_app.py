...
with col1:
    ...
    redemption_year = st.number_input("Ownership Duration (years)", min_value=1, value=5)
...
with result_col1:
    ...
    st.markdown(f"If you don't sell the house, your net position is  **${total_savings_invested + loan_amount - cumulative_interest_paid - hoa_paid - maintenance_paid - outstanding_principal:,.0f}**.")
    st.markdown("</div>", unsafe_allow_html=True)
...
with result_col2:
    ...
    st.markdown(
        f"You rent at **${int(cosmetics_rent)}** and invest the remaining of the monthly budget. Your rent increases by **{inflation}%** every year.")
    st.markdown(
